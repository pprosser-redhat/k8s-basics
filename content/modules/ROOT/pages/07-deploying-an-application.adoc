...
persistentvolumeclaim/national-parks-database created (dry run)
...
persistentvolumeclaim/national-parks-database created
...
As you are running on an environment provisioned specifically for the workshop, it does mean that we have to make a a change to the application deployment before we can go ahead and deploy the frontend application. 

==== Change the host address
In the terminal window, edit the file `frontend/ingress-parksmap.yaml`
...
[source, execute]
----
vim frontend/ingress-parksmap.yaml
----
...
Visit the front end web application by typing (or copying), or right clicking and opening in a new tab.
...
To load the data into the database, run the following `curl` command in your terminal window:
