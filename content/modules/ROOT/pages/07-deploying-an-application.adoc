...
We've deployed the frontend to our Parksmap application and learned some of the fundamentals of how applications are deployed and exposed in a Kubernetes cluster. So far, we have taken an imperative approach to configuring our application - specific commands executed with a correct order. 
...
We are going to deploy the remaining components of the Parksmap application. Kubernetes manifests for MongoDB and the backend application are held in a publically available Github repository, you need to clone it. To do this, in the terminal window enter the following:
